...
class DiffusionLoss(nn.Module):
    ...
    def forward(self, logits, labels, p_mask, total_tokens):
    ...
    def forward_old(self, logits, labels, masked_indices, p_mask, attention_mask):
        ...
        token_loss = F.cross_entropy(
            logits_flat[masked_indices_flat],
            labels_flat[masked_indices_flat],
            reduction='none'
        ) / p_mask_flat[masked_indices_flat]

        loss = token_loss.sum() / attention_mask_flat.sum().clamp(min=1)
        return loss

class FlexBertForDiffusionLM(FlexBertPreTrainedModel):
    def __init__(self, config: FlexBertConfig):
        super().__init__(config)
        ...
        self.loss_fn = DiffusionLoss(ignore_index=-100) # TODO not sure if we should add this as a config option or leave as is
        self.masked_loss_fn = nn.CrossEntropyLoss() if not hasattr(config, "loss_function") else get_loss_fn(config)
        ...
        **kwargs,
        ...
        if labels is not None:
            ...
            if self.return_z_loss:
                ...
                loss = self.loss_fn(logits, labels, p_mask, total_tokens)
