...
class SequencePacker(ABC):
    def __init__(
        ...
        world_size: int = 1,
        use_diffusion_masking: bool = False,
        ...
        self.use_diffusion_masking = use_diffusion_masking
    ...
    # TODO: Does this apply the masking properly when considering the padding?
    @staticmethod
    def diffusion_masking(
                seq: np.ndarray,
                mask_token: int,
                pad_token: int = -1,
        ...
        b, l = seq.shape
        # Sample a per-sequence masking probability
        t = np_rng.random(b)
        p_mask = (1 - eps) * t + eps
        p_mask = np.repeat(p_mask[:, None], l, axis=1)

        # Only mask non-padding tokens
        non_pad_mask = (seq != pad_token)
        masked_indices = (np_rng.random((b, l)) < p_mask) & non_pad_mask
        noisy_batch = np.where(masked_indices, mask_token, seq)

        # Set labels: original tokens, but ignore_index for padding
        labels = np.where(masked_indices, seq, ignore_index)

        if masked_indices is None:
            print('here')
        

        return noisy_batch, p_mask, labels
    ...
    def _generate_batches(self):
        ...
        while True:
            ...
            elif self.use_diffusion_masking:
                masked_batch, p_mask, labels = SequencePacker.diffusion_masking(
                    batch, self.mask_token_id, self.pad_token_id, self.ignore_token_id, self.np_rng)
                yieldval = {
    ...
    def mlm_masking(
        ...
        # Create mask: mask_prob of the time, mask the token
        mask_mask = np_rng.random(seq.shape) < mask_prob

        # Only compute loss over the tokens marked for masking
        labels = np.where(mask_mask, labels, ignore_index)
...
class GreedyBestFitSequencePacker(SequencePacker):
    ...
    def from_composer(
        ...
        batch_size_warmup_tokens: Optional[Union[str, Time]] = None,
        world_size: int = 1,
        use_diffusion_masking=False,      # <--- pass here
        diffusion_eps=1e-3,  
        ...
        return cls(
            ...
            use_diffusion_masking=use_diffusion_masking,
...
def split_packed_batch(batch: Any, microbatch_size: Union[int, float], padding_tolerance=1.0) -> Sequence:
    ...
    # Add these lines to handle diffusion masking keys
    has_masked_indices = "masked_indices" in batch
    has_p_mask = "p_mask" in batch
    if has_masked_indices:
        split_masked_indices = [x.squeeze() for x in batch["masked_indices"].split(1)]
    if has_p_mask:
        split_p_mask = [x.squeeze() for x in batch["p_mask"].split(1)]
    ...
    for i in range(num_items):
        ...
        if padding_amount > padding_tolerance:
            ...
            if has_masked_indices:
                masked_indices = split_masked_indices[i][: last_non_pad + 1]
            if has_p_mask:
                p_mask = split_p_mask[i][: last_non_pad + 1]
        else:
            ...
            if has_masked_indices:
                masked_indices = split_masked_indices[i]
            if has_p_mask:
                p_mask = split_p_mask[i]
