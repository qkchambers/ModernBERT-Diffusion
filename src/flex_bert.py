...
class DiffusionLanguageCrossEntropy(Metric):
    ...
    def update(self, logits: torch.Tensor, input_ids: torch.Tensor, masked_indices: torch.Tensor, p_mask: torch.Tensor):
        # Only compute loss for masked (non-padding) tokens
        token_loss = F.cross_entropy(
            logits[masked_indices],
            input_ids[masked_indices],
            reduction='none'
        ) / p_mask[masked_indices]
        self.sum_loss += token_loss.sum()
        self.total_items += masked_indices.sum()
...
def create_flex_bert_mlm(
    ...
    if pretrained_checkpoint is not None:
        ...
        if config.diffusion:
            model = bert_layers_module.FlexBertForDiffusionLM(config)
        else:
